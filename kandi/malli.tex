...
Tässä tutkimuksessa noudatetaan vahvasti IEEE:n määritelmiä \citep[s. 3-4]{ieee_2000} arkkitehtuuriselle termistölle seuraavalla tavalla:

\paragraph{Sidosryhmä (stakeholder)} Kuka tai ketkä tahansa, joilla on jokin rooli tai kiinnostus jotain asiaa kohtaan ohjelmistojärjestelmässä.

\paragraph{Arkkitehtuurinen kuvaus (architectural description)} Kokoelma asioita, jotka dokumentoivat arkkitehtuuria.

\paragraph{Komponentti} a.

\paragraph{Järjestelmä} Komponenttien joukko, jotka yhdessä mahdollistavat tietyn funktion tai kokoelman funktioita.

\paragraph{Arkkitehtuuri} Järjestelmän komponenttien ja niitä yhdistävien yhteyksien luoma joukko, jotka sijoittuvat tiettyyn ympäristöön.  

\paragraph{Näkymä} Kuvaus järjestelmästä niin, että se ottaa huomioon vain tietyt asiat.
...
Kommunikointi tärkeiden sidosryhmien kanssa on tärkeää, joista ääripäinä ovat kehittäjät ja asiakkaat. Kehittäjät haluavat usein todella tarkkaa usean eri tason ja näkökulman abstraktia kuvausta luotavasta järjestelmästä \citep[s. 120]{Ran:1998:ASV:288408.288438} tai ymmärtääkseen jo valmiiksi luotua järjestelmää \cite{ARaHVeSCiSDu}, kun taas asiakkaat haluavat nähdä järjestelmän toiminnallisuuden juuri heitä koskevien asioiden kannalta ja usein mahdollisimman korkealla tasolla.

\subsubsection{Näkökulmat}
Rozanskin ja ym. \citep[s. 36-42]{Rozanski:2011:SSA:2072649} määrittelevät, että näkökulmat (viewpoints) liittyvät vahvasti arkkitehtuurisiin näkymiin. Näkökulma voidaan määritellä olevan kokoelma malleja (patterns), templaatteja sekä konventioita, joiden avulla näkymä pystytään rakentamaan.  Näkökulmien joukko voidaan taas jakaa seitsemään eri osaan: kontekstinen, funktionaalinen, informaatiollinen, samanaikaisuudellinen, kehittämisellinen, käyttöönotollinen ja operationaalinen. Näistä keskitytään tämän tutkimuksen myöhemmässä vaiheessa kehittämisen sekä funktionaalisuuden näkökolmaan. Kehittämisen näkökulmaan siksi, koska se kommunikoi niiden sidosryhmien kanssa, jotka ovat mukana järjestelmän kehityksessä, testauksessa, ylläpidossa ja parantamisessa. Funktionaalinen näkökulma taas kuvailee muunmuassa järjestelmän suorituksenaikaisia komponentteja sekä niiden rajapintoja.
...
\subsection{Laadullisten vaatimusten keräys}
\subsection{Toiminnalliset ja ei-toiminnalliset vaatimukset}
