...
Testattavuus luetaan kuuluvan ylläpidettävyyden alle (Characteristics of Software Quality). Mitä on testaus järjestelmissä? 
\\

Yksikkö vs integraatio vs regressio testauksessa.
\\

Testaus löytää viat, testattavuus paikat jossa viat voivat sijaita \citep{Voas:1995:STN:624607.625469}. -> "testattavuus tukee testausta" vertauksena kullankaivuu.

Testatavuuden tarkoitus (realisaatio) on arvioida ohjelmaa siten, että voidaan määritellä onko järjestelmän laatu hyvä \citep[s. 20]{Voas:1995:STN:624607.625469}.
...
Mittaaminen on mahdollista tehdä ilman lähdekoodia, jos näkymä on jo valmiiksi luotu ja sen avulla voidaan tuoda ongelmakohtia esille ohjelmistojärjestelmässä, koska moni metriikoista määrittää järjestelmän sisäistä riippuvuusastetta. Tietyt komponentit nimittäin voivat olla suunniteltu pidettäväksi erillään, mutta silti analyysin seurauksena voidaan löytää ongelmakohtia.  

\subsubsection{Metrics for testability}

Tutkimalla testattavuutta alemmalla tasolla voidaan olio-pohjaisista järjestelmistä havaita seuraavat testaukseen vaikuttavat tekijät \citep[s. 5]{Dubey:2011:AMM:2020976.2020983}: metodien määrä luokassa; kaikki metodit luokkahierarkiassa, jotka suoritetaan "in response to a message to an object of that class or by some method in that class"; luokkien määrä, johon luokalla on kytkentää; periytymisaste eli kuinka monta luokkaa on luokan periytymishierarkiassa. 

Valitsemalla näistä samat mittarit, jotka leikkaavat ylläpidettävyyden kanssa, niin testattavuuden kannalta relevanteiksi, myös arkkitehtuuritasolla näkyviksi, saadaan: metodien määrä luokassa; peritymisaste; rajapinnan tarjoavien metodien koko; kytkentäaste muihin luokkiin; metodien määrä, jotka luokkaan kohdistuvien pyyntöjen toimesta luokkahierarkiassa. Kaikki nämä ovat havaittavissa järjestelmän rakenteellisessa mallissa, jossa komponentit ovat mallinnettu ja yhteydet luotu. "Miksi edellämainitut huonontavat testattavuutta?"
